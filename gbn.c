...
RECV:
	...
	/* if a connection teardown request is received, reply with FINACK header */
		...
		if (sendto(sockfd, &rec_header, sizeof(rec_header), 0, s.senderServerAddr, s.senderSocklen) == -1) return -1;
		s.state = FIN_RCVD;
		return 0;
	} else {
		goto RECV;
...
int gbn_connect(int sockfd, const struct sockaddr *server, socklen_t socklen){
	...
	while (attempt < MAX_ATTEMPT) {
		...
		if (check_packetType(rec_header, SYNACK) == 0) {
			...
			sendto(sockfd, &send_header, sizeof(send_header), 0, s.receiverServerAddr, s.receiverSocklen);
			return 0;
		}
	}
...
int gbn_accept(int sockfd, struct sockaddr *client, socklen_t *socklen){
	...
	*/
	...
	while (attempt < MAX_ATTEMPT) {
		...
		if (!syned) {
			if (maybe_recvfrom(sockfd, (char *)&send_header_syn, sizeof(send_header_syn), 0, s.senderServerAddr, &s.senderSocklen) == -1) {
				printf("error rec syn from sender\n");
				return -1;
			}
